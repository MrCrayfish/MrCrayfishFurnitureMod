...
public class BlockOutdoorTable extends BlockTable
{
	...
	public BlockOutdoorTable()
	{
		super(Material.WOOD, SoundType.WOOD, "table_outdoor");
    ...
    public void addBoxes(IBlockState state, World world, BlockPos pos, List<AxisAlignedBB> boxes)
    {
		...
		{
			if (!west)
				boxes.add(NORTH_WEST_LEG);
			if (!east)
				boxes.add(NORTH_EAST_LEG);
		}
		if (!south)
		{
			if (!west)
				boxes.add(SOUTH_WEST_LEG);
			if (!east)
				boxes.add(SOUTH_EAST_LEG);
		}
		boxes.add(TOP);
    }
