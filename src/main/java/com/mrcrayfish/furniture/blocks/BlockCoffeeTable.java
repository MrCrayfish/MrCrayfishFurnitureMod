...
public class BlockCoffeeTable extends Block
{
	...
	public static final AxisAlignedBB NORTH_EAST_LEG = new AxisAlignedBB(14.4 * 0.0625, 0, 0, 1, 1, 1.6 * 0.0625);
	...
	private List<AxisAlignedBB> getCollisionBoxList(IBlockState state)
	{
		...
		if (!north) {
			if (!west)
				list.add(NORTH_WEST_LEG);
			if (!east)
				list.add(NORTH_EAST_LEG);
		}
		if (!south) {
			if (!west)
				list.add(SOUTH_WEST_LEG);
			if (!east)
				list.add(SOUTH_EAST_LEG);
		}
		return list;
	}
