...
//	public static final int		DDS_FLAGS_VOLUME		= DDSCAPS2_VOLUME;
	...
	protected DDSHeader(ByteBuffer header) throws IOException
	{
		if(header.capacity() != 124)
			throw new IOException("DDS header needs 124 bytes");

		dwSize 			= header.getInt();
		if(dwSize != 124)
			throw new IOException("DDS header size is incorrect");
		...
		{
			throw new IOException("Required DDS flags missing!");
		}
		
		if(!hasCapsTexture) 
		{
			throw new IOException("Required DDS caps missing!");
		}
	}
}
