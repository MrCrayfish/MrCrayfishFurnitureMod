...
public class BlockEsky extends BlockFurnitureTile
{
	...
	{
		int meta = ((EnumFacing)worldIn.getBlockState(pos).getValue(FACING)).getHorizontalIndex();
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		if(playerIn.isSneaking())
		{
			...
			if((Boolean) state.getValue(OPENED))
			{
			...
			worldIn.setBlockState(pos, state.withProperty(OPENED, !((Boolean)state.getValue(OPENED))));
			if(tileEntity != null)
			{
				tileEntity.validate();
	...
	{
		return ((EnumFacing) state.getValue(FACING)).getHorizontalIndex() + ((Boolean) state.getValue(OPENED) ? 4 : 0);
	}
